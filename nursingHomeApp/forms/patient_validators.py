...
# error messages
VISITS_REQUIRED = """The dates of the last 2 visits are required when adding a
patient in Long Term or Skilled Care."""
PHYSICIAN_VISIT_REQUIRED = """To add a patient in Long Term Care, it is required
that one of the last 2 visits have been done by a Physician."""
...
NUM_COLS_REQUIRED = "Rows must be 11 fields long. Row %s has only %s fields."
...
LAST_VISIT_MISSING = "Cannot have a second to last visit,but not a last visit."
INVALID_LAST_VISIT = "Last visit must come after second to last visit."
...
ADMIT_DATE_AFTER_PV = """Admittance date cannot be after second to last visit
date."""


def validate_status_allowed(form, field):
    """lv == Last Vist, pv == Second to Last Visit"""
    ...
    for i, row in enumerate(reader, 2):
        ...
        if status in {'long term care', 'skilled care'} and (not lv or not pv):
            form.upload.data.seek(0)
            raise ValidationError(VISITS_REQUIRED)
        elif status == 'long term care' and not lvBy and not pvBy:
            form.upload.data.seek(0)
            raise ValidationError(PHYSICIAN_VISIT_REQUIRED)
    form.upload.data.seek(0)
...
def validate_status(form, field):
    ...
    for i, row in enumerate(reader, 2):
        first, last, room, status, md, np, lv, lvBy, pv, pvBy, admittance = row
        status = ' '.join(x for x in status.split(' ') if x).lower()
        if status not in VALID_PATIENT_STATUS:
            form.upload.data.seek(0)
...
def validate_required_fields(form, field):
    ...
    next(reader)
    for i, row in enumerate(reader, 2):
        first, last, room, status, md, np, lv, lvByD, pv, pvByD, admit = row
...
def gt_prior_visit(form, field):
    if form.priorVisit.data and not form.lastVisit.data:
        ...
        if form.priorVisit.data >= form.lastVisit.data:
            raise ValidationError(INVALID_LAST_VISIT)
...
def check_if_required(form, field):
    ...
    if form.status.data == 1:
        if not form.lastVisitBy.data and not form.priorVisitBy.data:
            raise ValidationError(PHYSICIAN_VISIT_REQUIRED)
...
def can_status_change(form, field):
    if form.status.data in {1, 2}:
        ...
        if len(visits) != 2:
            raise ValidationError(VISITS_REQUIRED)
        if form.status.data == 1 and not any(v[0] for v in visits):
            raise ValidationError(PHYSICIAN_VISIT_REQUIRED)
